...
class Tensor(object):
    ...
    def max_pool2d_cpp(self, kernel_size, stride, padding):
        ...
        output_max_inds = co.maxpool2d_forward_mp(self.data.astype(np.float32), output_max, kernel_size, stride, padding)
    ...
    def backward(self, grad=None, child_grad_node=None):
        ...
        if self.creator is not None and self.check_creator_grad_count():
            if self.create_op is None:
                ...
                new_grad = co.maxpool2d_backward_mp(self.grad.data.astype(np.float32), self.output_max_inds, \
